...
Replace both `replace-me` image tags in `kustomization.yaml` with the commit SHA. Replace the frontend origin in `configmap.yaml`. The frontend image itself must already be built with the public gateway API URL; changing a Pod environment variable cannot change a Vite build-time URL.
...
Then route the public frontend host to `devs-frontend:3000`, and add the gateway `/devs/api/**` route to `devs-service:8080`. The gateway and Mux must reach the webhook path. Managed PostgreSQL and provider egress must be reachable from the service Pods.
